...
export default function Notes() {
    ...
    const loadNotes = useCallback(async () => {
        const { data, error } = await fetchApi<BackendNote[]>(`/api/v1/note/read?audience=${audience}`);
        if (error) {
        ...
        <div className="flex h-full flex-col bg-zinc-950 text-zinc-50 p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-zinc-400 mt-1">Capture your thoughts, ideas, and reminders.</p>
                </div>
                    ...
                    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                        ...
                        <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-50 sm:max-w-md">
                            ...
                            <div className="grid gap-4 py-4">
                                ...
                                <Button onClick={handleCreateNote} className="bg-indigo-600 hover:bg-indigo-500 text-white w-full mt-2">Save Note</Button>
                            </div>
                        </DialogContent>
                    </Dialog>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 content-start pb-8">
                {notes.map((note) => (
                    <div key={note.id} className="relative group bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors h-fit shadow-sm shadow-zinc-900/50">
                        <div className="flex justify-between items-start mb-3 gap-2">
                            <h3 className="font-semibold text-lg leading-tight line-clamp-2">{note.title}</h3>

                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-8 w-8 -mr-2 -mt-1 text-zinc-400 hover:text-zinc-50 hover:bg-zinc-800 shrink-0">
                        ...
                        <p className="text-zinc-400 text-sm whitespace-pre-wrap">{note.description}</p>
                        <div className="mt-4 pt-4 border-t border-zinc-800/50 flex justify-between items-center text-xs font-medium">
                            <span className={`px-2 py-1 rounded-md ${note.target_audience === 'Him' ? 'bg-blue-900/30 text-blue-400' :
                                note.target_audience === 'Her' ? 'bg-pink-900/30 text-pink-400' :
