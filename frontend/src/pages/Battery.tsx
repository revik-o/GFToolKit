...
import { Battery, BatteryCharging, BatteryWarning, BatteryFull, BatteryMedium, Zap } from 'lucide-react';
...
export default function SocialBattery() {
    ...
    const loadBattery = useCallback(async () => {
        const { data, error } = await fetchApi<{ level: number }>(`/api/v1/social/battery/read?audience=${audience}`);
        if (error) {
    ...
    const getBatteryIcon = (level: number) => {
        if (level > 70) return <BatteryFull size={120} className="text-emerald-500 opacity-20" />;
        if (level > 30) return <BatteryMedium size={120} className="text-yellow-500 opacity-20" />;
        if (level > 0) return <BatteryWarning size={120} className="text-red-500 opacity-20" />;
        ...
        <div className="flex h-full flex-col bg-zinc-950 text-zinc-50 p-6 md:p-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
                <div>
                    ...
                    <p className="text-zinc-400 mt-1">Check in on each other's energy levels in real-time.</p>
                </div>

                <div className="flex items-center rounded-md bg-zinc-900 p-1 border border-zinc-800">
                    <Button variant="ghost" size="sm" onClick={() => setAudience('Him')} className={audience === 'Him' ? 'bg-blue-900/50 text-blue-300' : 'text-zinc-400'}>Him's Battery</Button>
                    <Button variant="ghost" size="sm" onClick={() => setAudience('Her')} className={audience === 'Her' ? 'bg-pink-900/50 text-pink-300' : 'text-zinc-400'}>Her's Battery</Button>
                </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-10 items-center justify-center flex-1 max-w-5xl mx-auto w-full">
                ...
                <div className="flex-1 w-full max-w-md flex flex-col items-center justify-center p-8 bg-zinc-900/40 rounded-3xl border border-zinc-800 relative overflow-hidden">
                    ...
                    <div className="relative z-10 w-48 h-80 rounded-[2rem] border-8 border-zinc-800 bg-zinc-950 p-2 flex flex-col justify-end shadow-2xl relative overflow-hidden">
                        ...
                        >
                            {batteryLevel === 100 && (
                                <Zap size={48} className="text-emerald-950 opacity-40 animate-pulse absolute" />
                            )}
                        </div>

                        <div className="absolute inset-0 flex items-center justify-center mix-blend-difference pointer-events-none">
                            <span className="text-4xl font-black text-white">{batteryLevel}%</span>
                        </div>
                    </div>

                    <p className="mt-8 text-lg font-medium text-zinc-300 text-center relative z-10 transition-colors">
                        {batteryLevel > 70 ? "Ready for anything!" :
                            batteryLevel > 30 ? "Doing okay, might need a break soon." :
                                batteryLevel > 0 ? "Running on fumes, need quiet time." :
                ...
                <div className="flex-1 w-full max-w-md flex flex-col gap-3">
                    <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-500 mb-2">Update Level</h3>
                    ...
                    >
                        <span className="flex items-center gap-3"><BatteryCharging size={18} /> Fully Charged</span>
                    ...
                    >
                        <span className="flex items-center gap-3"><BatteryFull size={18} /> High Energy</span>
                    ...
                    >
                        <span className="flex items-center gap-3"><BatteryMedium size={18} /> Something In Between</span>
                    ...
                    >
                        <span className="flex items-center gap-3"><BatteryMedium size={18} className="opacity-70" /> Halfway There</span>
                    ...
                    >
                        <span className="flex items-center gap-3"><BatteryWarning size={18} /> Running Low</span>
                    ...
                    >
                        <span className="flex items-center gap-3"><Battery size={18} className="text-red-500" /> Completely Drained</span>
