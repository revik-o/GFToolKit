...
export default function Goals() {
    ...
    const loadGoals = useCallback(async () => {
        const { data, error } = await fetchApi<BackendGoal[]>(`/api/v1/goal/read?audience=${audience}`);
        if (error) {
        ...
        <div className="flex h-full flex-col bg-zinc-950 text-zinc-50 p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-zinc-400 mt-1">Track shared ambitions and personal milestones.</p>
                </div>
                        ...
                        <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-50 sm:max-w-md">
                            ...
                            <div className="grid gap-4 py-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <Select value={newPriority} onValueChange={(v) => setNewPriority(v as Goal['priority'])}>
                                ...
                                <Button onClick={handleCreateGoal} className="bg-indigo-600 hover:bg-indigo-500 text-white w-full mt-2">Save Goal</Button>
                            </div>
                        </DialogContent>
                    </Dialog>
                </div>
            ...
            <div className="space-y-4 pb-8 max-w-4xl">
                {goals.map((goal) => (
                    <div key={goal.id} className="group bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-5 hover:bg-zinc-900 hover:border-zinc-700 transition-all flex flex-col sm:flex-row gap-4 sm:items-center">
                        ...
                        <div className="hidden sm:flex h-12 w-12 rounded-full bg-zinc-800/80 items-center justify-center shrink-0 border border-zinc-700/50">
                            <Target className={`h-6 w-6 ${goal.target_audience === 'Him' ? 'text-blue-400' :
                                goal.target_audience === 'Her' ? 'text-pink-400' :
                                    'text-indigo-400'
                                }`} />
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start mb-1 gap-2">
                                <h3 className="font-semibold text-lg text-zinc-100 truncate">{goal.title}</h3>
                                <div className="sm:hidden">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800 text-zinc-50 w-32">
                                            ...
                                            <DropdownMenuItem className="text-red-400 hover:text-red-300 hover:bg-red-900/50 cursor-pointer" onClick={() => handleDelete(goal.id)}><Trash2 size={14} className="mr-2" /> Delete</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </div>
                            </div>

                            <p className="text-zinc-400 text-sm line-clamp-2 sm:line-clamp-1 mb-3 sm:mb-2">{goal.description}</p>

                            <div className="flex flex-wrap items-center gap-3 text-xs font-medium">
                                <Badge variant="outline" className={`${priorityColors[goal.priority]} bg-transparent`}>
                                    {goal.priority} Priority
                                </Badge>

                                <span className={`px-2 py-0.5 rounded-md ${goal.target_audience === 'Him' ? 'bg-blue-900/20 text-blue-400 border border-blue-900/30' :
                                    goal.target_audience === 'Her' ? 'bg-pink-900/20 text-pink-400 border border-pink-900/30' :
                                        'bg-zinc-800/50 text-zinc-300 border border-zinc-700/50'
                                    }`}>
                                    {goal.target_audience}
                                </span>
                                <span className="text-zinc-600 ml-auto sm:ml-0">
                                    {new Date(goal.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                                </span>
                            </div>
                        </div>
