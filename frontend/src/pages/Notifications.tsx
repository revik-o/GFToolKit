...
export default function Notifications() {
    ...
    const triggerTestNotification = async (type: 'info' | 'success' | 'warning' = 'info') => {
        try {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
                <div>
                    ...
                    <p className="text-zinc-400 mt-1 flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${isConnected ? 'bg-emerald-900/30 text-emerald-400 border border-emerald-800/50' : 'bg-red-900/30 text-red-400 border border-red-800/50'}`}>
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => triggerTestNotification('info')} className="bg-zinc-900 border-zinc-800 hover:bg-zinc-800 text-zinc-300">
                        Test Info
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => triggerTestNotification('success')} className="bg-emerald-900/20 border-emerald-900/50 hover:bg-emerald-900/40 text-emerald-400">
                        Test Success
                    </Button>
                </div>
            </div>
            ...
            <div className="flex-1 space-y-4">
                {notifications.length === 0 ? (
                    ...
                    notifications.map(notif => (
                        <div key={notif.id} className="relative group flex p-4 bg-zinc-900/60 border border-zinc-800 rounded-xl transition-all hover:bg-zinc-900 hover:shadow-lg hover:-translate-y-0.5 animate-in fade-in slide-in-from-top-4 duration-300">
